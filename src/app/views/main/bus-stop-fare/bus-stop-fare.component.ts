...
export class BusStopFareComponent {
    ...
    selectedBusStop$: Observable<IBusStop | null>;
    ...
    handleUpdateBusStop(status?: string): void {
        if (status === 'cancel') {
            ...
            this.selectedBusStop$.pipe(takeUntil(this.destroy$)).subscribe((_bs: IBusStop | null) => {
                if (_bs !== null) {
                    this.store.dispatch(selectBusStopForFare({ payload: _bs.id }));
                    this.router.navigate(['/main']);
                }
            });
        }
